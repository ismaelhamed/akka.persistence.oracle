...
namespace Akka.Persistence.Sql.Common.TestKit
{
    public abstract class SqlJournalQuerySpec : PluginSpec
    {
        ...
        {
            private static readonly IDictionary<Tuple<string, long>, DateTime> KnownEventTimestampMappings = new Dictionary<Tuple<string, long>, DateTime>
            {
        ...
        {
            return $@"{plugin}.timestamp-provider = ""{typeof(TestTimestampProvider).FullName}, {typeof(TestTimestampProvider).Assembly.GetName().Name}""";
        }
